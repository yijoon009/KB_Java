...
--사원이름, 사원번호, 부서이름
--equl join이용, 서로 다른 테이블에 자료를 외래키로 연결된 것으로 이용하여 검색할 때
...
select s.name as "학생이름", p.name as "교수이름" from student s, professor p where s.profno = p.profno;
select s.name as "학생이름", p.name as "교수이름" from student s join professor p on s.profno = p.profno;

--학생테이블, 학과테이블(department), 교수 테이블 조인
--학생이름, 학생 학과, 학생 지도교수 이름 출력
select s.name as "학생이름" , d.dname as "학과명", p.name as "지도교수 이름" 
...
--student 테이블 1전공이 101번인 학생의 이름과 각 학과별 지도교수 이름을 출력
select s.deptno1, s.name as "학생이름", p.name as "교수이름"
...
--고객별로 마일리지 포인트 조회한 후 해당 마일리지 점수로 받을 수 있는 상품을 조회
--고객의 이름, 받을 수 있는 상품명을 출력
select c.gname as "고객명", to_char(c.point, '999,999') as "마일리지",g.gname as "상품명" 
...
--student 테이블과 score 테이블, hakjum 테이블을 조회하여 학생들의 이름, 점수, 학점을 출력
select * from student;
select * from score;
select * from hakjum;
...
--outer join
--student 테이블과 professor 테이블을 조인하여 학생이름과 지도교수 이름을 출력
--단, 지도교수가 결정되지 않은 학생의 명단도 출력하시오
...
--student 테이블과 professor 테이블을 조인하여 학생이름과 지도교수 이름을 출력
--단, 지도학생이 결정 안된 교수명단, 지도교수가 결정 안된 학생명단을 한꺼번에 출력
select s.name, p.name from student s, professor p where s.profno(+) = p.profno
union
select s.name, p.name from student s, professor p where s.profno = p.profno(+);
